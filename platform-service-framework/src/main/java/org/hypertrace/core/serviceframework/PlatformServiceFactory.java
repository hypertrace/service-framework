 ...
 *
 * The main.class configuration property in the config file is used for this.
 */
public class PlatformServiceFactory {

  private static final Logger LOGGER = LoggerFactory
      .getLogger(PlatformServiceFactory.class);
  ...
  public static synchronized PlatformService get(ConfigClient configClient) {
    ...
    try {
      Class<PlatformService> serviceClass = (Class<PlatformService>) Class
          .forName(mainClass);
      return serviceClass.getDeclaredConstructor(ConfigClient.class).newInstance(configClient);
    } catch (Exception e) {
      LOGGER.error("PlatformService main class could not be found - main.class = {}",
          mainClass, e);
      System.err.println(String
          .format("PlatformService main class could not be found - main.class = %s", mainClass));
      e.printStackTrace();
